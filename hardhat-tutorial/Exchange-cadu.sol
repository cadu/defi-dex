...
contract Exchange is ERC20 {
  ...
  function addLiquidity(uint _amount) public payable returns (uint) {
    ...
    * If the reserve is empty, intake any user supplied value for `Ether` and `Crypto Dev` tokens because there is no ratio currently
    ...
    if (cryptoDevTokenReserve == 0) {
      ...
      /*
      If the reserve is not empty, intake any user suppplied value for `Ether`and determine according to the ratio how many `Crypto Dev` tokens need to be supplied to prevent any large price impacts because of the additional liquidity
      */
      ...
      // transfer only (cryptoDevTokenAmount user can dd) amount of `Crypto Dev tokens` from user's account
      // to the contract
  ...
  * in the swap
  ...
  function removeLiquidity(uint _amount) public returns (uint, uint) {
    ...
    // = (Current Eth reserve * amount of LP tokens that user wnats to withdraw)/Total supply of `LP` tokens
    uint ethAmount = (ethReserve * _amount) / _totalSupply;
    // The amount of Crypto Dev token that wuld be sent back to the user is based
    // on a ratio
    // Ratio is -> (Crypto Dev sent back to the user/Current Crypto Dev token reserve)
    // = (amount of LP tokens that use wants to withdraw)/Total supply of `LP` tokens
  ...
  * in the swap
  ...
  function getAmountOfTokens(
    ...
    // Δx = ((input amount)*99)/100, x = inputReserver, y = outputReserve
